...
> \SplFileObject rebConversionConvertToExcel($reb_file, $write_prices, $write_long_texts, $conversion_culture, $include_article_numbers, $lock_all_cells_but_prices)
...
try {
    $result = $apiInstance->rebConversionConvertToExcel($reb_file, $write_prices, $write_long_texts, $conversion_culture, $include_article_numbers, $lock_all_cells_but_prices);
    print_r($result);
} catch (Exception $e) {
...
> \Dangl\AVACloud\Model\FlatAvaProject rebConversionConvertToFlatAva($reb_file)
